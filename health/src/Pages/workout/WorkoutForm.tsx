...
const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
  ...
  const newWorkout: WorkoutType = {name, exercises: newExercises, progress: 0 };
  onSubmit(newWorkout);
  setName('');
  ...
  return (
    <Box borderWidth="1px" borderRadius="lg" p="4" width={"70%"}  m={"50px auto"} >
      <Heading as="h2" size="lg" mb="2" color={"white"}>
        Create New Workout
      </Heading>
      <form onSubmit={handleSubmit} color={"white"}>
        <Input type="text" color={"white"} placeholder="Workout Name" value={name} onChange={handleNameChange} mb="2" />
        {exercises.map((exercise, index) => (
          <Box key={index} mb="2" color={"white"}>
            <Input
              type="text"
